    ...
    async def upcoming(interaction: discord.Interaction):
        try:
            ...
            for evt in filtered[:10]:  # Show max 10 events
                ...
                # Set banner and thumbnail safely
                if event_banner and (not embed.image or not embed.image.url):
                    embed.set_image(url=event_banner)
                if creator_avatar and (not embed.thumbnail or not embed.thumbnail.url):

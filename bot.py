...
# ---------- /mark command ----------
@bot.tree.command(
    name="mark", description="Staff only: Create a Mark Attendance embed from a TruckersMP event link."
)
@app_commands.describe(
...
async def mark(interaction: discord.Interaction, event_link: str, channel: discord.TextChannel, color: str = "blue"):
    ...
    # Creator object may have 'avatar' or 'logo'
    vtc_avatar = None
    ...
    # Convert meetupDateTime to UTC and NPT
    if event_start:
        evt = event_start
        if evt.endswith("Z"):
            evt = evt[:-1]
    ...
    embed.set_footer(text="Powered by NepPath | Time 24hrs")
